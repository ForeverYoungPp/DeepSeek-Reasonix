...
export function registerChoiceTool(
  ...
  registry.register({
    name: "ask_choice",
    description:
      "CALL THIS WHENEVER your next reply would end with 'A / B / C — 你选哪个?' or any enumerated list of alternatives plus 'let me know / tell me your preference / 你决定'. Text menus DO NOT WORK in this TUI — writing options as prose leaves the user with no picker, they have to scroll back and type a letter manually, worse UX than if you'd just made the call yourself. The tool fires a magenta picker modal; user picks and the loop resumes with 'user picked <id>'. TRIGGERS YOU MUST SELF-DETECT: (1) you just finished analyzing 2–4 approaches and were about to ask the user to pick; (2) the user explicitly asked for options to choose from ('列出方案', '给我几个选择', 'what are my options'); (3) you're at a genuine preference fork (library, naming, style, strategic direction) where the user has context you don't. Keep option ids short (A/B/C or option-1/option-2). Each needs a title; summary is optional but helps. Set allowCustom=true when the user's real answer might not fit your list. Max 6 options — if you have more, narrow first. SKIP this tool only when: (a) one option is obviously correct — just do it or submit_plan it, don't pretend to ask; (b) you just need a text answer (clarification, ambiguous reference) — ask in plain prose. When in doubt about whether to present options as text or call the tool, CALL THE TOOL.",
    readOnly: true,
    parameters: {
      type: "object",
      properties: {
